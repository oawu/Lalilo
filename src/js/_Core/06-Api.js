...
void (_ => {
  ...
  window.Api.prototype.setThen = function (funcs = null) {
    if (T.arr(funcs)) {
      this._thens = funcs.filter(func => T.func(func) || T.asyncFunc(func) || T.promise(func))
    }
    return this
  }
  window.Api.prototype.then = function (func = null) {
    if (T.arr(func)) {
      return this.setThen(func)
    }
  ...
  const _send = function (closure) {
    return promisify(closure, async _ => {
      ...
      for (const then of thens) {
        ...
        if (T.func(then)) {
          result = await tryFunc(_ => then(data))
        }
        if (T.asyncFunc(then)) {
          result = await tryFunc(async _ => await then(data))
        }
        if (T.promise(then)) {
          result = await tryFunc(then)
        }
        if (T.err(result)) {
          throw result
        }
