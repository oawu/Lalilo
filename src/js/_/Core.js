...
const Flash = function(key, message) {
  if (!(this instanceof Flash)) {
    return new Flash(key, message)
  }

  const data = Data.get(Flash.key) || {}

  if (Array.isArray(data[key])) {
    data[key].push(message)
  } else {
    data[key] = [message]
  }
...
const Load = {
  ...
  Auth: option => Auth()
    ? Load.Vue(option)
    : Auth.logout(_ => Change('login', Flash.Toastr.failure('請重新登入！')))
}
...
const BaseURL = (path = '', query = null, hash = null) => {
  if (BaseURL.func === undefined) {
    BaseURL.func = (splitter, object, className) => {
      ...
      if (typeof object != 'object' || Array.isArray(object) || object === null || object === undefined) {
        return ''
      }
  ...
  if (typeof path == 'object' && !Array.isArray(path) && path !== null && path !== undefined) {
    hash = query
    query = path
    path = null
  }
  ...
  if (query instanceof Param._.Object.Hash) {
    hash = query
    query = null
  }
  ...
  return `${/(http(s?)):\/\//.test(path)
    ? path
    : `${window.location.protocol}//${window.location.host}/${path.split('/').map(t => t.trim()).filter(t => t !== '').join('/') || 'index'}${new RegExp(`html$`).test(path) ? '' : 'html'}`}${BaseURL.func('?', query, Param._.Object.Query)}${BaseURL.func('#', hash, Param._.Object.Hash)}`
}
...
// Auth
const Auth = function() {
  if (Auth._.user && Auth._.user instanceof Auth) {
    return Auth._.user
  }
  ...
  return Auth._.user = ((data, token) => {
    const auth = {}
    for (let key in data) {
      Object.defineProperty(auth, key, {
        get () { return data[key] }
      })
    }
...
API.prototype.auth = function(token) {
  return this.header('Authorization', `Bearer ${token || Auth() || ''}`)
}
...
API.prototype.send = function() {
  ...
  .fail(({ status: code, responseJSON = null }) => {
    ...
    if (!Lib.Helper.Type.isObject(responseJSON)) {
      responseJSON = {}
    }
    if (Lib.Helper.Type.isString(responseJSON.message) && !Array.isArray(responseJSON.messages)) {
