...
const TableView = {
  ...
  Section: {
    ...
    template: `<div :class="{ _section: true, __group: isGroup, __clear: isClear }"><div v-if="$slots.header" class="_header"><slot name="header"></slot></div><span v-else-if="header !== ''" class="_header">{{ header }}</span><div class="_body"><slot></slot></div><div v-if="$slots.footer" class="_footer"><slot name="footer"></slot></div><span v-else-if="footer !== ''" class="_footer">{{ footer }}</span></div>`
  },
  Cell: {
    props: {
    ...
    computed: {
      ...
      className () {
        ...
        types.push(`__title_color_${['black', 'gray', 'red', 'orange', 'yellow', 'green', 'blue', 'purple', 'brown'].includes(this.titleColor)
          ? this.titleColor
          : 'black'}`)
        ...
        types.push(`__subtitle_color_${['black', 'gray', 'red', 'orange', 'yellow', 'green', 'blue', 'purple', 'brown'].includes(this.subtitleColor)
          ? this.subtitleColor
          : 'gray'}`)
        types.push(`__subtitle_align_${['left', 'center', 'right'].includes(this.subtitleAlign)
          ? this.subtitleAlign
          : 'left'}`)

        types.push(`__value_color_${['black', 'gray', 'red', 'orange', 'yellow', 'green', 'blue', 'purple', 'brown'].includes(this.valueColor)
          ? this.valueColor
          : 'gray'}`)
        types.push(`__value_align_${['left', 'center', 'right'].includes(this.valueAlign)
          ? this.valueAlign
          : 'right'}`)

        types.push(`__title_wrap_${this.titleWrap
          ? 'y'
          : 'n'}`)
        types.push(`__subtitle_wrap_${this.subtitleWrap
          ? 'y'
          : 'n'}`)
        types.push(`__value_wrap_${this.valueWrap
          ? 'y'
          : 'n'}`)

        if (typeof this.radioUI == 'boolean') {
          types.push(`__radio_${this.radioUI
            ? 'on'
            : 'off'}`)
        }

        if (this.switchUI !== undefined) {
          types.push(`__switch${this.switchUI !== null
            ? ''
            : '_loading'}`)
        }

        if (typeof this.imageUI == 'string' && this.imageUI !== '') {
          types.push('__image')
        }
        return types
      },
      style () {
        ...
        style['--tbv-cell-image-width'] = isNaN(w)
          ? this.imageUIWidth
          : `${w}px`

        let h = parseFloat(this.imageUIHeight)
        style['--tbv-cell-image-height'] = isNaN(h)
          ? this.imageUIHeight
          : `${h}px`

        let r = parseFloat(this.imageUIRadius)
        style['--tbv-cell-image-radius'] = isNaN(r)
          ? this.imageUIRadius
          : `${r}px`
    ...
    methods: {
      click () {
        ...
        if (this.arrowUI === true) {
          this.$emit('arrowUI', this.arrowUI)
          return this
        }
        if (sw && !ro) {
  ...
  InputText: {
    ...
    template: `
    <label :class="className" @click="click">
