...
module.exports = {
  ...
  build: async _ => {
    ...
    await cli(`複製 JavaScript 檔案`, async cli => {
      cli.cmdSubtitle('執行動作', `copy .js files`)
      ...
      for (const file of files.jsFiles) {
        ...
        const result = fs.writeFile(file.dist.file, content, { encoding: 'utf8' })
        if (T.err(result)) {
          throw new Error(`無法寫入 ${Path.$.rRoot(file.dist.file)}`, { cause: result })
        }
        cli.advance()
    ...
    await cli(`編譯後${['複製', Config.isMinify ? '壓縮' : '', Config.isMerge ? '合併' : ''].filter(t => t !== '').join('、')} Html 檔案`, async cli => {
      ...
      for (const file of files.htmlFiles) {
        ...
        const result = fs.writeFile(file.dist.file, Config.isMinify ? MinifyHtml(content, { collapseWhitespace: true, continueOnParseError: false }) : content, { encoding: 'utf8' })
        if (T.err(result)) {
          throw new Error(`無法寫入 ${Path.$.rRoot(file.dist.file)}`, { cause: result })
        }
        cli.advance()
